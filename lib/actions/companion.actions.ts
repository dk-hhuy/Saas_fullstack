...
export const addToSessionHistory = async (companionId: string) => {
    ...
    const { data, error } = await supabase
        ...
        .select();  // ← Thêm .select() để trả về data đã insert

    if (error) throw new Error(error.message || 'Failed to add to session history');

    return data[0];  // ← Trả về record đã insert
}
...
export const getRecentSession = async (limit = 10) => {
    ...
    const { data, error } = await supabase
        .from('session_history')
        .select(`companions:companion_id (*), created_at`)
        .order('created_at', { ascending: false })
        .limit(limit);

    if (error || !data) throw new Error(error?.message || 'Failed to fetch recent sessions');

    // Trả về tất cả sessions với thông tin session
    return data.map(({ companions, created_at }) => ({
        ...(companions as any),
        sessionDate: created_at,
        sessionId: `${(companions as any).id}-${created_at}`
    }));
}
